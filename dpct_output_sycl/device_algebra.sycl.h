...
NOTES:
 ...
 # SYCL_TASK is considered completed successfully if the value of the .task_error field equals zero.
...
#define MAX_SYCL_TASKS 128 //max allowed number of simultaneously active SYCL tasks per SYCL device
#ifdef GPU_FINE_TIMING
#define NUM_EVENTS_PER_TASK 6 //number of SYCL events recorded per SYCL task
#else
#define NUM_EVENTS_PER_TASK 4 //number of SYCL events recorded per SYCL task
#endif
#define MAX_SYCL_EVENTS (MAX_SYCL_TASKS*NUM_EVENTS_PER_TASK) //max number of SYCL events per SYCL device
...
#define MAX_SYCL_BLOCKS 1024       //max number of SYCL thread blocks per kernel
#if SYCL_ARCH >= 300
...
//SYCL TASK STATUS (keep consistent with tensor_algebra.F90):
#define SYCL_TASK_ERROR -1
#define SYCL_TASK_EMPTY 0
#define SYCL_TASK_SCHEDULED 1
#define SYCL_TASK_STARTED 2
#define SYCL_TASK_INPUT_THERE 3
#define SYCL_TASK_OUTPUT_THERE 4
#define SYCL_TASK_COMPLETED 5
...
typedef struct dpct_type_257494 {
...
typedef struct dpct_type_4e4b36 {
...
// SYCL task (returned by non-blocking SYCL functions):
typedef struct dpct_type_7e004f {
 ...
 int queue_hl;           //SYCL queue handle the task went into
 int event_start_hl;     //handle of the SYCL event recorded at the beginning of the task
 int event_comput_hl;    //handle of the SYCL event recorded before the SYCL kernels start (all input data is on Device)
 int event_output_hl;    //handle of the SYCL event recorded when the SYCL kernels finish (before output to the Host)
 int event_finish_hl;    //handle of the SYCL event recorded at the end of the task (full completion)
 ...
 syclTensArg_t tens_args[MAX_TENSOR_OPERANDS]; //tensor arguments participating in the tensor operation
 void * pref_ptr;        //tensor operation prefactor location
} cudaTask_t;
//Note: Adding new SYCL events will require adjustment of NUM_EVENTS_PER_TASK.

// Device statistics:
typedef struct dpct_type_f738ec {
 ...
 std::chrono::steady_clock::time_point time_start; //time when the library was initialized (internal use only)
} talsh_stats_t;
...
//  NV-TAL SYCL task API:
 ...
 int sycl_task_wait(cudaTask_t *sycl_task);
 int sycl_tasks_wait(unsigned int num_tasks, cudaTask_t **sycl_tasks, int *task_stats);
 int sycl_task_error_code(const cudaTask_t *sycl_task);
 int sycl_task_dev_rsc_copy(const cudaTask_t *sycl_task, unsigned int arg_num, char which, talsh_dev_rsc_t *dev_rsc);
 int sycl_task_dev_rsc_move(cudaTask_t *sycl_task, unsigned int arg_num, char which, talsh_dev_rsc_t *dev_rsc);
 int sycl_task_arg_has_resource(cudaTask_t *sycl_task, unsigned int arg_num, char which, int *ierr);
 int sycl_task_arg_destroy(cudaTask_t *sycl_task, int arg_num = -1);
 float sycl_task_time(const cudaTask_t *sycl_task, float *in_copy = NULL, float *out_copy = NULL, float *comp = NULL,
                      float *mmul = NULL);
 float sycl_task_time_(const cudaTask_t *sycl_task, float *in_copy, float *out_copy, float *comp, float *mmul);
 void sycl_task_print(const cudaTask_t *sycl_task);
//  NV-TAL tensor operations:
 int gpu_tensor_block_place(tensBlck_t *ctens, int gpu_id, unsigned int coh_ctrl, cudaTask_t *sycl_task, void *dev_mem = NULL);
 int gpu_tensor_block_init(tensBlck_t *dtens, double val_real, double val_imag, unsigned int coh_ctrl, cudaTask_t *sycl_task, int gpu_id = -1);
 int gpu_tensor_block_slice(tensBlck_t *ltens, tensBlck_t *dtens, const int * offsets,
                            unsigned int coh_ctrl, cudaTask_t *sycl_task, int gpu_id = -1, int accumulative = NOPE);
 int gpu_tensor_block_insert(tensBlck_t *ltens, tensBlck_t *dtens, const int * offsets,
                             unsigned int coh_ctrl, cudaTask_t *sycl_task, int gpu_id = -1, int accumulative = NOPE);
 int gpu_tensor_block_copy(const int *cptrn, tensBlck_t *ltens, tensBlck_t *dtens, unsigned int coh_ctrl, cudaTask_t *sycl_task,
                           int gpu_id = -1, int conj_bits = 0);
 int gpu_tensor_block_add(const int *cptrn, tensBlck_t *ltens, tensBlck_t *dtens, unsigned int coh_ctrl, cudaTask_t *sycl_task,
 ...
 int gpu_tensor_block_contract_dlf(const int *cptrn, tensBlck_t *ltens, tensBlck_t *rtens, tensBlck_t *dtens,
                                   unsigned int coh_ctrl, cudaTask_t *sycl_task, int gpu_id = -1,
