...
typedef struct dpct_type_998f6a {
...
static size_t *abh_occ=nullptr; //occupation status for each buffer entry in Host argument buffer (*arg_buf_host)
...
int arg_buf_allocate(size_t *arg_buf_size, int *arg_max, int gpu_beg,
                     int gpu_end)
    /** This function initializes all argument buffers on the Host and GPUs in
    the range [gpu_beg..gpu_end]. INPUT: # arg_buf_size - requested size of the
    page-locked Host argument buffer in bytes; # [gpu_beg..gpu_end] - range of
    GPUs assigned to the current MPI process; OUTPUT: # arg_buf_size - actual
    size of the allocated page-locked Host argument buffer in bytes; # arg_max -
    max number of arguments the Host buffer can contain (those of the lowest
    size level).
    **/
    try {
 ...
 *arg_max=0; abh_occ=nullptr; abh_occ_size=0; max_args_host=0; arg_buf_host_size=0;
 for(i=0;i<MAX_GPUS_PER_NODE;i++){abg_occ[i]=nullptr; abg_occ_size[i]=0; max_args_gpu[i]=0; arg_buf_gpu_size[i]=0;}
//Allocate the Host argument buffer:
...
#ifndef NO_GPU
  /*
  DPCT1048:0: The original value cudaHostAllocPortable is not meaningful in the
  ...
  */
  err = (arg_buf_host =
             (void *)sycl::malloc_host(hsize, dpct::get_default_queue()),
         0);
  if (err != 0) {
   hsize-=mem_alloc_dec;
  }else{
...
//Initialize the Host argument buffer occupancy tables:
  abh_occ=(size_t*)malloc(hsize*sizeof(size_t)); if(abh_occ == nullptr) return 2; //Host buffer occupancy table
  abh_occ_size=hsize;
...
//Allocate GPUs buffers, if needed:
   ...
   */
   /*
   DPCT1000:6: Error handling if-stmt was detected but could not be rewritten.
   */
   /*
   DPCT1003:7: Migrated API does not return error code. (*, 0) is inserted. You
   may need to rewrite this code.
   */
   err = (i = dpct::dev_mgr::instance().device_count(), 0);
       if (err != 0) return 6;
...
// Global memory banks for each GPU:
    ...
    for(i=gpu_beg;i<=gpu_end;i++){
      ...
      */
      /*
      DPCT1000:9: Error handling if-stmt was detected but could not be
      rewritten.
      */
      /*
      ...
      */
      err = (dpct::dev_mgr::instance().select_device(i), 0);
          if (err != 0) return 9;
      /*
      DPCT1010:15: SYCL uses exceptions to report errors and does not use the
      error codes. The call was replaced with 0. You need to rewrite this code.
      */
      err = 0; // check for pre-existing CUDA errors for this GPU
      /*
      ...
      */
       ...
       */
       err_msg = "cudaGetErrorString not supported" /*cudaGetErrorString(err)*/;
       if(VERBOSE) printf("#ERROR(TALSH:mem_manager:arg_buf_allocate): CUDA pre-error: %s\n",err_msg);
       return 10;
      }
      err=cudaMemGetInfo(&hsize,&total);
      /*
      DPCT1000:13: Error handling if-stmt was detected but could not be
      rewritten.
      */
      if (err != 0) {
       /*
       ...
       */
       err = (arg_buf_gpu[i] =
                  (void *)sycl::malloc_device(hsize, dpct::get_default_queue()),
              0);
       if (err != 0) {
        hsize-=mem_alloc_dec;
       }else{
...
// Initialize each GPU argument buffer occupancy tables:
       abg_occ[i]=(size_t*)malloc(hsize*sizeof(size_t)); if(abg_occ[i] == nullptr) return 12; //GPU#i buffer occupancy table
...
int arg_buf_deallocate(int gpu_beg, int gpu_end)
    /** This function deallocates all argument buffers on the Host and GPUs in
       the range [gpu_beg..gpu_end] **/
    try {
 int i,err_code;
#ifndef NO_GPU
 int err = 0;
...
#pragma omp flush
 err_code=0;
 if(abh_occ != nullptr) free(abh_occ); abh_occ=nullptr; abh_occ_size=0; max_args_host=0;
 for(i=0;i<MAX_GPUS_PER_NODE;i++){
  if(abg_occ[i] != nullptr) free(abg_occ[i]); abg_occ[i]=nullptr; abg_occ_size[i]=0; max_args_gpu[i]=0;
 }
 ...
 may need to rewrite this code.
 */
 err = (sycl::free(arg_buf_host, dpct::get_default_queue()), 0);
 /*
  ...
  for(i=gpu_beg;i<=gpu_end;i++){
   if(i < MAX_GPUS_PER_NODE){
    if(gpu_is_mine(i) != 0){
     ...
     You may need to rewrite this code.
     */
     err = (dpct::dev_mgr::instance().select_device(i), 0); if (err == 0) {
      ...
      You may need to rewrite this code.
      */
      err = (sycl::free(arg_buf_gpu[i], dpct::get_default_queue()), 0);
      /*
      DPCT1000:24: Error handling if-stmt was detected but could not be
      rewritten.
      */
      if (err != 0) {
       if(VERBOSE) printf("\n#ERROR(mem_manager:arg_buf_deallocate): GPU# %d argument buffer deallocation failed!",i);
       /*
       DPCT1001:23: The statement could not be removed.
       */
       err_code++;
      }
     }else{
...
#else
 free(arg_buf_host); arg_buf_host=nullptr;
#endif /*NO_GPU*/
 bufs_ready=0;
#pragma omp flush
...
RETURN STATUS:
 # 0 - success (*entry_num>=0, *entry_ptr!=nullptr);
...
{
 *slab=nullptr; *slab=(slab_t*)malloc(sizeof(slab_t)); if(*slab == nullptr) return -1;
...
{
 slab->max_entries=0; slab->entry_size=0; slab->slab_base=nullptr; slab->free_entries=nullptr;
 return 0;
}

#ifndef NO_GPU
int slab_construct(slab_t *slab, size_t slab_entry_size,
                   size_t slab_max_entries, size_t align, int mapped)
#else
int slab_construct(slab_t *slab, size_t slab_entry_size,
                   size_t slab_max_entries, size_t align)
#endif
 ...
 if(slab == nullptr || slab_entry_size == 0 || slab_max_entries == 0) return -1;
 slab->slab_base=nullptr; slab->free_entries=nullptr; slab->max_entries=0;
 if(align == 0){
 ...
 if(slab->free_entries == nullptr){slab->entry_size=0; return 1;}
#ifndef NO_GPU
 if(mapped == 0){
  ...
  /*
  DPCT1048:1: The original value cudaHostAllocPortable is not meaningful in the
  migrated code and was removed or replaced with 0. You may need to check the
  migrated code.
  */
  /*
  DPCT1048:2: The original value cudaHostAllocMapped is not meaningful in the
  migrated code and was removed or replaced with 0. You may need to check the
  migrated code.
  */

  slab->slab_base = (void *)sycl::malloc_host((slab->entry_size) * slab_max_entries, dpct::get_default_queue());
  if (err == 0) { slab->mem_mapped = 1; } else { slab->slab_base = nullptr; }
 }
#else
 slab->slab_base=(void*)malloc((slab->entry_size)*slab_max_entries);
#endif
 if(slab->slab_base == nullptr){
...
/** Gets a slab entry. **/
{
 if(slab == nullptr) return -1;
 if(slab->max_entries == 0 || slab->slab_base == nullptr || slab->free_entries == nullptr) return -2;
...
{
 if(slab == nullptr) return -1;
 if(slab->slab_base == nullptr) return -2;
 if(base_ptr == nullptr) return -3;
...
/** Returns the max number of entries in the slab (capacity). **/
{
 if(slab == nullptr) return -1;
 if(slab->slab_base == nullptr) return -2;
 if(max_entries == nullptr) return -3;
...
/** Returns the slab entry size in bytes. **/
{
 if(slab == nullptr) return -1;
 if(slab->slab_base == nullptr) return -2;
 if(entry_size == nullptr) return -3;
...
/** Returns the byte offset of a given slab entry from the slab base. **/
{
 ...
 if(slab == nullptr) return -1;
 if(slab->max_entries == 0 || slab->slab_base == nullptr) return -2;
 if(slab_entry_p == nullptr) return -3;
 if(entry_offset == nullptr) return -4;
...
int slab_destruct(slab_t *slab) try {
/** Destructs a slab. **/
    int errc;
#ifndef NO_GPU
    int err;
	...
	if(slab->mem_mapped == 0){
	    free(slab->slab_base); slab->slab_base=nullptr;
	}else{
	    cl::sycl::free(slab->slab_base, dpct::get_default_queue());
	    if (err != 0) errc = NOT_CLEAN;
	}
#else
	free(slab->slab_base); slab->slab_base=nullptr;
#endif
    ...
    if(slab->free_entries != nullptr){
	if(slab->max_entries == 0) errc=NOT_CLEAN;
	free(slab->free_entries); slab->free_entries=nullptr;
    }else{
	if(slab->max_entries > 0){slab->max_entries=0; errc=NOT_CLEAN;}
    }
    slab->max_entries=0;
...
int host_mem_alloc(void **host_ptr, size_t tsize, size_t align)
{
	...
	if(*host_ptr == nullptr) return TRY_LATER;
    }
    return 0;
}
...
int host_mem_alloc_pin(void **host_ptr, size_t tsize) try {
#ifndef NO_GPU
    *host_ptr = (void *)cl::sycl::malloc_host(tsize, dpct::get_default_queue());
...
int host_mem_free_pin(void *host_ptr) try {
#ifndef NO_GPU
    cl::sycl::free(host_ptr, dpct::get_default_queue());
    if (err != 0) return 1;
#else
    free(host_ptr); host_ptr=nullptr;
#endif
    return 0;
}
catch (cl::sycl::exception const &exc) {
...
int host_mem_register(void *host_ptr, size_t tsize) try {
#ifndef NO_GPU
    /*
      DPCT1027:36: The call to cudaHostRegister was replaced with 0, because DPC++
...
int host_mem_unregister(void *host_ptr) try {
#ifndef NO_GPU
 /*
 DPCT1027:38: The call to cudaHostUnregister was replaced with 0, because DPC++
 currently does not support registering of existing host memory for use by
 device. Use USM to allocate memory for use by host and device.
 */
 int err = 0;
...
#ifndef NO_GPU
int gpu_mem_alloc(void **dev_ptr, size_t tsize, int gpu_id) try {
...
/** Allocates global memory on a specific GPU (or current GPU by default).
	...
	*/
	/*
	  DPCT1000:41: Error handling if-stmt was detected but could not be rewritten.
	*/
	err = i = dpct::dev_mgr::instance().current_device_id();
	if (err != 0) return 1;
	/*
	...
	*/
	/*
	  ...
	  may need to rewrite this code.
	*/
	err = (dpct::dev_mgr::instance().select_device(gpu_id), 0); 
	if (err != 0) {
	    err = (dpct::dev_mgr::instance().select_device(i), 0); return 2;
	}
    }

    *dev_ptr = (void *)cl::sycl::malloc_device(tsize, dpct::get_default_queue());
    /*
...
int gpu_mem_free(void *dev_ptr, int gpu_id)
  ...
  */
  ...
  may need to rewrite this code.
  */
  err = (dpct::dev_mgr::instance().select_device(gpu_id), 0); if (err != 0) {
   err = (dpct::dev_mgr::instance().select_device(i), 0); return 2;
  }
 }
 /*
 ...
 may need to rewrite this code.
 */
 err = (sycl::free(dev_ptr, dpct::get_default_queue()), 0);
 /*
 DPCT1001:55: The statement could not be removed.
 */
 /*
 DPCT1000:56: Error handling if-stmt was detected but could not be rewritten.
 */
 /*
