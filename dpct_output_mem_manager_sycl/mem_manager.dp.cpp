...
typedef struct dpct_type_d5da76 {
...
int slab_construct(slab_t *slab, size_t slab_entry_size,
    ...
    //#else
    // int slab_construct(slab_t * slab, size_t slab_entry_size, size_t
    // slab_max_entries, size_t align) #endif
...
//#ifndef NO_GPU
  ...
  */
  ...
  may need to rewrite this code.
  */
  err = ((slab->slab_base) = (void *)sycl::malloc_host(
             (slab->entry_size) * slab_max_entries, dpct::get_default_queue()),
         0);
  if (err == 0) { slab->mem_mapped = 1; } else { slab->slab_base = NULL; }
 }
//#else
 slab->slab_base=(void*)malloc((slab->entry_size)*slab_max_entries);
//#endif
...
//#ifndef NO_GPU
  if(slab->mem_mapped == 0){
   ...
   /*
   DPCT1001:27: The statement could not be removed.
   */
   /*
   DPCT1000:28: Error handling if-stmt was detected but could not be rewritten.
   */
   /*
   DPCT1003:29: Migrated API does not return error code. (*, 0) is inserted. You
   may need to rewrite this code.
   */
   err = (sycl::free(slab->slab_base, dpct::get_default_queue()), 0);
...
//#ifndef NO_GPU
 /*
 DPCT1048:3: The original value cudaHostAllocPortable is not meaningful in the
 migrated code and was removed or replaced with 0. You may need to check the
 migrated code.
 */
 /*
 ...
 */
 int err =
     (*host_ptr = (void *)sycl::malloc_host(tsize, dpct::get_default_queue()),
      0);
     if (err != 0) return 1;
//#else
 ...
 */
 ...
 may need to rewrite this code.
 */
 int err = (sycl::free(host_ptr, dpct::get_default_queue()), 0);
     if (err != 0) return 1;
//#else
 ...
 */
 err =
     (*dev_ptr = (void *)sycl::malloc_device(tsize, dpct::get_default_queue()),
      0);
 /*
