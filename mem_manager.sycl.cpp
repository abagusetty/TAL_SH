...
#ifndef NO_GPU
      arg_buf_host = cl::sycl::malloc_host(hsize, talsh::get_queue());
...
// Global memory banks for each GPU:
		...
		for(i=gpu_beg;i<=gpu_end;i++){
		    if(gpu_is_mine(i) != 0){ //Initialize only my GPUs
		      talsh::set_device(i);
		      total = talsh::get_queue().get_device().get_info<cl::sycl::info::device::global_mem_size>()
...
/** Constructs a user-defined slab. **/
{
 ...
 if(mapped == 0){
  ...
  if(err == cudaSuccess){slab->mem_mapped=1;}else{slab->slab_base=nullptr;}
 }
#else
 slab->slab_base=(void*)malloc((slab->entry_size)*slab_max_entries);
#endif
...
#else
    *host_ptr=(void*)malloc(tsize); if(*host_ptr == nullptr) return 1;
#endif
    return 0;
}
